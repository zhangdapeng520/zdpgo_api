...
func (g *Gin) CreateToken(claims Claims) (string, error) {
	// 过期时间默认3小时
	claims.ExpiresAt = time.Now().Add(time.Second * time.Duration(g.config.JwtExpired)).Unix()

	// 创建token
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)

	// 返回token
	g.log.Info("使用的key是什么", "key", g.config.JwtKey)
	return token.SignedString([]byte(g.config.JwtKey))
}
...
func (g *Gin) ParseToken(tokenString string) (*Claims, error) {
	...
	token, err := jwt.ParseWithClaims(tokenString, &Claims{}, func(token *jwt.Token) (i interface{}, e error) {
		return []byte(g.config.JwtKey), nil
	})
