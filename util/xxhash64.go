...
// 实现了 xxHash 的 64 位变体 (XXH64)，如 http://cyan4973.github.io/xxHash/ 所述，不依赖任何标准库软件包

// XXHash64 计算字符串的 xxhash，返回uint6的hash值
func XXHash64(content string) uint64 { return sum([]byte(content)) }

// sum 计算字节数组的 xxhash，返回uint6的hash值
