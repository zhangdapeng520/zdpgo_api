...
// Throttle is a middleware that demo07_limits number of currently processed requests
...
// ThrottleBacklog is a middleware that demo07_limits number of currently processed
...
// ThrottleWithOpts is a middleware that demo07_limits number of currently processed requests using passed ThrottleOpts.
func ThrottleWithOpts(opts ThrottleOpts) func(http.Handler) http.Handler {
	if opts.Limit < 1 {
		panic("api/middleware: Throttle expects limit > 0")
	}

	if opts.BacklogLimit < 0 {
		panic("api/middleware: Throttle expects backlogLimit to be positive")
	}
...
// throttler demo07_limits number of currently processed requests at a time.
